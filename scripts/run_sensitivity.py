...
Quantizes each transformer block individually to INT8 and measures
accuracy impact compared to the FP32 baseline.
"""
...
def run():
    ...
    texts, labels = load_sst2(sample_size=200)
    print(f"Loaded {len(texts)} samples\n")

    # Use block-level granularity for meaningful results
    layers = get_quantizable_layers(model, granularity="block")
    print(f"Analyzing {len(layers)} components...\n")
